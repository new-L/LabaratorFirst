...
public class Logic 
    ...
    {
        double S, p = 3.141592653589793;
        S = p * Math.pow(radius,2.0);
        return S;
    }
    public static double circleCircumference(double radius)//Длина окружности
    {
        double leng, p = 3.141592653589793;
        leng = 2 * p * radius;
        return leng;
    }
    public static double rightTriangleArea(double base, double height)//Площадь прямоугольного треугольника
    {
        double S;
        S = 0.5 * base * height;
        return S;
    }
    public static double rightTrianglePerimeter(double base, double height)//Периметр прямоугольного треугольника
    {
        double c,P;
        c = Math.pow(base,2.0) + Math.pow(height,2.0);
        double flag;
        flag = Math.sqrt(c);
        P = flag + base + height;
        return P;
    }
    public static double boxVolume(double length, double width, double depth)//Объем куба
    {
        double V;
        V = length * width * depth;
        return V;
    }
    public static double boxSurfaceArea(double length, double width, double depth) //Площадь поверхности куба
    {
        //Total Surface Area = 2(Area hw ) + 2(Area hl ) + 2(Area wl )
        double S;
        S = 2 * ((depth * width)+(depth * length)+(width * length));
        return S;
    }
    public static double sphereVolume(double radius)//Обьем шара(сферы)
    {
        double V, p =3.141592653589793;
